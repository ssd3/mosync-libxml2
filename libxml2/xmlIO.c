...
static const char *IOerr[] = { "Unknown IO error", /* UNKNOWN */
...
"flush error", "write error", "no input", "buffer full", "loading error",
...
void __xmlLoaderErr(void *ctx, const char *msg, const char *filename) {
	...
	__xmlRaiseError(schannel, channel, data, ctxt, NULL, XML_FROM_IO,
			XML_IO_LOAD_ERROR, level, NULL, 0, filename, NULL, NULL, 0, 0, msg,
			filename);
...
//				&content_type, content_encoding, content_lgth);
//
//		if (http_ctxt != NULL) {
//#ifdef DEBUG_HTTP
 ...
 * Returns the new parser output or NULL
 */
xmlOutputBufferPtr xmlAllocOutputBufferInternal(
...
#ifdef LIBXML_OUTPUT_ENABLED
xmlOutputBufferPtr __xmlOutputBufferCreateFilename(const char *URI,
...
xmlOutputBufferPtr xmlOutputBufferCreateBuffer(xmlBufferPtr buffer,
	...
	ret = xmlOutputBufferCreateIO((xmlOutputWriteCallback) xmlBufferWrite,
			(xmlOutputCloseCallback) NULL, (void *) buffer, encoder);
...
int xmlParserInputBufferRead(xmlParserInputBufferPtr in, int len) {
	...
	else if (xmlBufGetAllocationScheme(in->buffer)
...
int xmlOutputBufferFlush(xmlOutputBufferPtr out) {
	...
	if ((out->conv != NULL) && (out->encoder != NULL)
		...
		ret = out->writecallback(out->context,
				(const char *) xmlBufContent(out->conv), xmlBufUse(out->conv));
		if (ret >= 0)
			xmlBufShrink(out->conv, ret);
	...
	if ((pref != XML_CATA_ALLOW_NONE) && (!xmlNoNetExists(URL))) {
		...
		if ((ctxt != NULL) && (ctxt->catalogs != NULL)
			...
			resource = xmlCatalogLocalResolve(ctxt->catalogs,
					(const xmlChar *) ID, (const xmlChar *) URL);
		}
		/*
		 * Try a global lookup
		 */
